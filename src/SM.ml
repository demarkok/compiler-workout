...
let compile (defs, p) = 
  ...
  let rec compile' stmt =
    ...
    | Language.Stmt.Case (to_match, branches) ->
      let end_label = nameGenerator#getName in 
      ...
      let rec match_pattern pat true_label false_label = (match pat with 
        ...
        | Stmt.Pattern.Sexp (tag, pts) ->
          ...
          [DROP] @
          [JMP true_label] @
          [LABEL new_false_label] @ [DROP] @ [JMP false_label]
      ) in
      ...
      let compile_branch (pat, body) =
        ...
        [LABEL true_label] @ (*здесь мы с одним исксом*)
        ...
        [LEAVE] @
        [JMP super_end_label] @
        [LABEL next_branch_label] @ (*здесь мы с двумя исксами*) 
        [DROP]
      in
